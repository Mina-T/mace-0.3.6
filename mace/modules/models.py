...
class MACE(torch.nn.Module):
    def __init__(
        ...
        embedding_mat: Optional[np.ndarray] = None,
    ):
        super().__init__()
        ...
        if isinstance(embedding_mat, np.ndarray):
            self.embmat = True
            self.register_buffer(
            "embedding_mat", torch.tensor(embedding_mat, dtype=torch.get_default_dtype())
            )
            emb_size = embedding_mat.shape[1]
            self.node_embedding = LinearNodeEmbeddingBlock(
                irreps_in=o3.Irreps(f'{emb_size}x0e'), irreps_out=node_feats_irreps
            )
        else:
            self.embmat = False
            self.node_embedding = LinearNodeEmbeddingBlock(
    ...
    def forward(
        ...
        if self.embmat:
            node_feats = self.node_embedding(
                torch.matmul(data["node_attrs"],self.embedding_mat))
        else:
